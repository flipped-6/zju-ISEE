...
void Oled_Write_Data_Inv(u8 data)
{
    IIC_Start();
    IIC_Send_Data(0x78);//地址
    IIC_Send_Data(0x40);//写数据
    IIC_Send_Data_Inv(data);//指令
...
//显示一个最大为999999的数字 对于index显示的数字翻转显示
void Oled_Display_Num_with_Index(uint8_t* num,uint8_t index,uint8_t row,uint8_t col)
{
    ...
    {
        if(i==index){
            Oled_Display_Char_Inv(row,col,num[i]+'0',1);
        }else{
            Oled_Display_Char_Inv(row,col,num[i]+'0',0);
        }
        col+=8;
    }
}
...
void Oled_Display_Char_Inv(u8 page,u8 col,char ch,u8 inv)
{
    ...
    {
        if(inv){
                Oled_Write_Data_Inv(Aciss_8X16[loca * 16 + i]);
        }else{
    ...
    {
        if(inv){
                Oled_Write_Data_Inv(Aciss_8X16[loca * 16 + 8 + i]);
        }else{
